      ...
      <div style={{ padding: isMobile ? 12 : 20, maxWidth: 1180, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "210px 1fr", gap: isMobile ? 12 : 18, alignItems: "start" }}>
          ...
          <main style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {/* 入力対象月の進め方・過去分修正 */}
            <Card id="month-advance" title="入力対象月の進め方・過去分の修正">
              <p style={p}>
                どの月が入力できるかは「日付」ではなく、<Pill>経過月数</Pill>という値で決まります。経過月数は <b>経営 ＞ 期(会計年度)マスタ</b> で管理する<b>全社共通の値</b>で、生産本部・経営の集計（年累計・平均・達成率・判定）の基準にもなります。
              </p>
              <Table head={["月の状態", "条件", "画面の見え方"]} rows={[
                ["入力対象月(1か月)", "経過月数 + 1 の月", "黄色セル。ここだけ入力可"],
                ["確定済み", "経過月数 以下の月", "グレーでロック。編集不可"],
                ["未到来", "経過月数 + 2 以降の月", "「―」表示。入力不可"],
              ]} />

              <p style={{ ...p, marginTop: 14, marginBottom: 6, fontWeight: 700, color: "#1f3864" }}>翌月分を入力可能にする（月を進める＝締める）</p>
              <ol style={{ fontSize: 13, margin: 0, paddingLeft: 18, lineHeight: 1.9 }}>
                <li>前月分の入力・保存が終わったら、<b>経営 ＞ 期(会計年度)マスタ</b> を開く。</li>
                <li>当期の行の<b>経過月数を +1</b> して保存する（例: 5月入力後に <b>9 → 10</b> で6月が入力可に）。</li>
                <li>KPI実績入力を再読込すると、対象月が次の月（黄色セル）に進む。</li>
              </ol>
              <p style={{ ...p, marginTop: 8, fontSize: 12, color: "#64748b" }}>
                ※ 月の前進は自動では行われません。毎月、前月分を入力したら経過月数を+1して締める運用です。
              </p>

              <p style={{ ...p, marginTop: 14, marginBottom: 6, fontWeight: 700, color: "#1f3864" }}>確定済み（過去）の月を修正する</p>
              <ol style={{ fontSize: 13, margin: 0, paddingLeft: 18, lineHeight: 1.9 }}>
                <li>期マスタで<b>経過月数を「直したい月の1つ前」まで下げて</b>保存する（その月が黄色＝入力可になる）。</li>
                <li>KPI実績入力で該当月を修正して保存する。</li>
                <li><b>経過月数を元の値に戻して</b>保存する（戻し忘れ厳禁）。</li>
                <li>再読込し、後ろの月と集計が元どおりに戻っていることを確認する。</li>
              </ol>
              <div style={{ fontSize: 12, padding: "10px 12px", borderRadius: 8, marginTop: 10, background: "#fff7ed", border: "1px solid #fed7aa", color: "#9a3412", lineHeight: 1.7 }}>
                <b>注意</b>
                <ul style={{ margin: "4px 0 0", paddingLeft: 18 }}>
                  <li>経過月数は<b>集計の打ち切り月</b>です。下げている間は、その月以降の入力済み実績が集計から外れ、<b>全社のダッシュボード・達成率・判定が一時的に途中までの値</b>になります。</li>
                  <li>下げても<b>後ろの月のデータは消えません</b>（画面で「―」になり隠れるだけ）。元に戻せば再表示されます。</li>
                  <li>全社共通マスタのため、<b>他の人が入力・閲覧していない時間帯に短時間で</b>行い、終わったら必ず経過月数を元に戻してください。</li>
                  <li>一度に編集できるのは入力対象の1か月だけです。複数月を直す場合は月ごとに繰り返します。修正は監査ログに記録されます。</li>
                </ul>
              </div>
            </Card>
            ...
            <Card id="terms" title="用語集">
              <Table rows={[
                ...
                ["経過月数", "実績が確定している月数。入力対象月(=経過月数+1)・進捗率・★・月割合算の基準。期マスタで管理。"],
