 ...
 * 営業「参考図台帳検索」メニュー権限付与。
 *
 * メニュー表示には L1(M003)・L2(M003-05)・プログラム(PGM046) の3レベル許可が必要。
 * M003/M003-05 は kijun-fusoku 登録時に 営業部/DX推進室 へ許可済み(冪等スキップ)。
 * 今回は新規プログラム PGM046 を同じ 営業部 + DX推進室 に付与する。
 *
 * 冪等: 既存(グループ名×対象種別×対象ID)があればスキップ。
 * 実行: npx tsx scripts/grant-eigyo-sankou-zu-permissions.ts [--dry-run]
 */
...
const GRANTS: Grant[] = [
  { group: "営業部", menus: ["M003", "M003-05"], programs: ["PGM046"] },
  { group: "DX推進室", menus: ["M003", "M003-05"], programs: ["PGM046"] },
];
