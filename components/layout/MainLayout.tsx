...
export function MainLayout({ children }: MainLayoutProps) {
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden print:h-auto print:overflow-visible">
      {/* グローバルヘッダー - 印刷時非表示 */}
      <header className="flex-shrink-0 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 shadow-lg z-50 no-print">
      ...
      <div className="flex-1 flex overflow-hidden relative print:overflow-visible">
        ...
        {sidebarOpen && (
          <div
            className="fixed inset-0 bg-black/30 z-40 transition-opacity no-print"
        ...
        {/* POPサイドバー - 印刷時非表示 */}
        <div
          className={`fixed top-0 left-0 h-full z-50 transform transition-transform duration-300 ease-in-out no-print ${
        ...
        {/* コンテンツ */}
        <main className="flex-1 overflow-hidden print:overflow-visible print:w-full">{children}</main>
      </div>
    </div>
  );
}
