/**
 * 営業メニューへ「参考図台帳検索」を登録する。
 *
 *   npx tsx scripts/register-eigyo-sankou-zu-menu.ts --dry-run
 *   npx tsx scripts/register-eigyo-sankou-zu-menu.ts
 *
 *   L1 M003 営業部（既存）
 *     └ L2 M003-05 支援ツール（kijun-fusoku が作成済み。無ければ作成）
 *         PGM045 基準風速積雪検索 /eigyo/kijun-fusoku（既存）
 *         PGMnnn 参考図台帳検索   /eigyo/sankou-zu  ← 今回追加
 *
 * program_id は機能配置マスタの既存最大値から自動採番(max+1)する(採番ミス防止)。
 * 既存の同一URL/同一名があればスキップ(冪等)。配置先は master base のメニュー表示/機能配置マスタ。
 * 注意: ユーザに表示するにはグループ権限/個別権限マスタでの許可設定が別途必要。
 */
...
const NEW_PROGRAM = { name: "参考図台帳検索", menu_id: "M003-05", url: "/eigyo/sankou-zu", sort: 2 };
